...
editProfileSave.addEventListener("click", async (event) => {
  ...
  await axiosInstance
    .post(`auth-header/profile/${userID}`, userForm, {
      headers: {
...
if (personal) {
  unfollow.addEventListener("click", async (event) => {
    ...
    await axiosInstance.delete(
      `auth-header/profile/${userID}/unfollow`,
      {
        data,
      },
      {
...
followersPreview.addEventListener("click", async () => {
  ...
  let followers = await axiosInstance.post(
    `auth-header/profile/:{id}/followers`,
    {
      user_id: userID,
    }
  );
...
followingBoard.addEventListener("click", async (event) => {
  ...
  let followings = await axiosInstance.post(
    `auth-header/profile/:{id}/followings`,
    {
      user_id: userID,
    }
  );
